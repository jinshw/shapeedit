...
func main() {
	...
	r.GET("/converCoordinate", func(context *gin.Context) {
		filename := context.Query("filename")
		Encoding = context.Query("encoding")
		srcFilepath := DirPathPrefix + filename
		pIndex := strings.LastIndex(srcFilepath, ".")
		distFilepath := srcFilepath[0:pIndex] + "_wgs84" + srcFilepath[pIndex:]

		flag := shpGCJ02toWGS84(srcFilepath, distFilepath)
...
//shape 文件从GCJ02（火星坐标系）=>WGS84
func shpGCJ02toWGS84(src string, dist string) int {
